...
function add_social_profile_links_after_title($title, $id) {
    ...
    if (is_single() && in_the_loop() && $id === get_the_ID()) {
        ...
        if ($linkedin_profile || $facebook_profile) {
            ...
            if ($linkedin_profile) {
                $title .= '<a href="' . esc_url($linkedin_profile) . '" target="_blank" rel="nofollow" class="linkedin-profile-link">LinkedIn Profile</a>';
            }
            if ($linkedin_profile && $facebook_profile) {
                $title .= ' | ';
            }
            if ($facebook_profile) {
                $title .= '<a href="' . esc_url($facebook_profile) . '" target="_blank" rel="nofollow" class="facebook-profile-link">Facebook Profile</a>';
            }
            $title .= '</div>';
        }
    }
